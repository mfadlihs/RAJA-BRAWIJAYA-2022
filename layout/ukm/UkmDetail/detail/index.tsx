			...
			<CloudLayer>
				<WhiteLayer className='relative pb-20'>
					<div className='lg:px-28 md:px-24 sm:px-16 px-6 pt-24 relative z-20'>
						...
						/>
						<div className='flex'>
							<TabUkm name='Deskripsi' value={0} isActive={defaultTab == 0} onChange={onChange}></TabUkm>
							<TabUkm name='Kegiatan' value={1} isActive={defaultTab == 1} onChange={onChange} />
							<TabUkm name='Prestasi' value={2} isActive={defaultTab == 2} onChange={onChange} />
							<TabUkm name='FAQ' value={3} isActive={defaultTab == 3} onChange={onChange} />
						</div>
						<div className='py-6'>
							{defaultTab === 0 && <UkmDescription desc={data.description} pesan={data.pesan} />}
							{defaultTab === 1 && <ActivityTab data={dummy.activity} />}
							{defaultTab === 2 && <AchievmentTab data={data.prestasi} />}
							{defaultTab === 3 && <FaqTab data={dummy.faq} />}
						</div>
					</div>
					<img src='/assets/images/ukm/wave.svg' className='w-screen absolute z-0 bottom-0' />
				</WhiteLayer>
			</CloudLayer>
		</div>
	);
